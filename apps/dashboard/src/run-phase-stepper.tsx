...
export function RunPhaseStepper(props: RunPhaseStepperProps) {
  ...
  return (
    <section className="run-tape" aria-label="Run progress">
      <ol className="stepper">
        {RUN_PHASES.map((phase, index) => {
          ...
          return (
            <li
              key={phase}
              className={`stepper-item is-${state}`}
              aria-current={state === "current" ? "step" : undefined}
            >
              <span className="run-tape-line" aria-hidden="true">
                <span className="run-tape-dot">
      ...
      <p className="run-operation" aria-live="polite">
        ...
        <strong>
          {props.detail.length > 0 ? props.detail : props.current}
        </strong>
      </p>
    </section>
  );
