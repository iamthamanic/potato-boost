...
This is a pnpm workspace. `packages/core` is a TypeScript smoke package. `packages/schemas` holds the canonical Zod + JSON Schema contracts for run artifacts. Remaining `apps/` and `packages/` from the PRD are still unscaffolded.
...
├── packages/             # core, schemas; remaining CLI/adapters planned
